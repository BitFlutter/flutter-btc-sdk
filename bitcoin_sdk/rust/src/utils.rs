...
/// Valida se um valor em satoshis é válido (não negativo e dentro dos limites)
...
pub fn format_btc_amount(satoshis: u64) -> String {
    let btc = satoshis_to_btc(satoshis);
    format!("{:.8}", btc)
}
...
pub fn parse_btc_amount(btc_str: &str) -> Result<u64, Box<dyn std::error::Error>> {
    ...
    let satoshis = btc_to_satoshis(btc);
    if !is_valid_amount(satoshis) {
...
mod tests {
    ...
    fn test_is_valid_amount() {
        ...
        assert!(is_valid_amount(21_000_000 * 100_000_000));
        assert!(!is_valid_amount(21_000_001 * 100_000_000));
    }
